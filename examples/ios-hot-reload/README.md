...
over the network when files change and reopen the app.
...
ls $(pwd)/build/Build/Products/Debug-iphonesimulator/${XCODE_TARGET_NAME}.app
```

## Start sync

```bash
export SYNC_FOLDER_PATH=$(pwd)/build/Build/Products/Debug-iphonesimulator/${XCODE_TARGET_NAME}.app
export LIM_API_KEY=<lim token from Console>
```

The following will set up the sync and print a link for you to access the iOS simulator.
