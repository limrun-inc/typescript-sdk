...
export default class IosBuild extends BaseCommand {
  static summary = 'Run xcodebuild on an Xcode sandbox';
  static description =
    'Sync a local project path once (or the current working directory if omitted), then trigger a remote xcodebuild with streaming output. This works with iOS instances that have `--xcode` enabled and can also target a standalone Xcode instance when you pass `--id`.';

  static examples = [
    '<%= config.bin %> ios build',
    '<%= config.bin %> ios build ./MyProject',
    '<%= config.bin %> ios build --id <ios-instance-ID>',
    '<%= config.bin %> ios build --id <ios-instance-ID> --scheme MyApp',
    '<%= config.bin %> ios build --id <xcode-instance-ID> --scheme MyApp --workspace MyApp.xcworkspace',
    '<%= config.bin %> ios build ./MyProject --project MyApp.xcodeproj --upload ios-build.zip',
  ];
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(IosBuild);
