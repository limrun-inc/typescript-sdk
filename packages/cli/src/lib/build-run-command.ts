...
export function buildRunCommand(platform: 'xcode' | 'gradle'): typeof BaseCommand {
  return class BuildRun extends BaseCommand {
    static summary = `Run a command on a ${platform} sandbox`;
    static description =
      'Sync the current directory, then run a one-shot shell command in the remote workspace with streamed output.';

    static examples = [
      `<%= config.bin %> ${platform} run -- make api`,
      `<%= config.bin %> ${platform} run apps/api -- make generate`,
      `<%= config.bin %> ${platform} run --env API_ENV=development -- npm run generate`,
      `<%= config.bin %> ${platform} run --no-sync -- mise run build`,
    ];
    ...
    static flags = {
      ...
      ...syncFlags,
      id: Flags.string({
        description: `${platform} instance ID to run on. Defaults to the most recent standalone ${platform} target.`,
      }),
      env: Flags.string({
    ...
    async run(): Promise<void> {
      ...
      if (delimiter < 0) {
        this.error(`Separate the remote command with --, for example: lim ${platform} run -- make api`);
      }
      const commandArgs = this.argv.slice(delimiter + 1);
      if (commandArgs.length === 0) {
        this.error('A command is required after `--`.');
      }
      const { args, flags } = await this.parse(BuildRun, this.argv.slice(0, delimiter));
      ...
      await this.withAuth(async () => {
        const { target, client } = await this.resolveBuildToolClient(platform, flags.id);
        ...
        if (platform === 'xcode' && /\bsimctl\b/.test(commandLine)) {
          this.info(
            `Note: the ${platform} sandbox does not host simulators, so simctl cannot install or launch apps here. ` +
        ...
        const proc = client.run(commandLine, {
          cwd: args.cwd ?? '.',
          ...(env && { env }),
