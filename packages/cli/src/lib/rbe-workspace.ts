 ...
 *   `127.0.0.1:${port}`), powering the live + persisted console; bazel appends
 *   the invocation id to `--bes_results_url` and prints the per-build link.
...
build:limrun --bes_results_url=${besResultsUrl}
...
export function writeRbeWorkspaceFiles(
  ...
  fs.writeFileSync(
    bazelrcFragment,
    renderLimrunBazelrc(port, xcodeVersionKey, isMacClient, bepFile, besResultsUrl),
  );
