...
describe('getBuildLogs', () => {
  ...
  test('reads the newest persisted build after the instance is deleted', async () => {
    const instanceMissing = Object.assign(new Error('instance not found'), { status: 404 });
    const records = [
      { id: 'build-10', status: 'SUCCEEDED', exitCode: 0, downloadUrl: 'https://logs/10' },
      { id: 'build-9', status: 'FAILED', exitCode: 1, downloadUrl: 'https://logs/9' },
    ];
    const result = await getBuildLogs({
      instanceId: 'xcode_deleted',
      follow: false,
      listPersisted: async () => records,
      observe: async () => {
        throw instanceMissing;
    ...
    expect(result).toMatchObject({
      execId: 'build-10',
      logs: 'downloaded https://logs/10\n',
      source: 'persisted',
    });
    expect(records[0]?.id).toBe('build-10');
  });
});
