...
```sh
git clone git@github.com:stainless-sdks/limrun-v1-typescript.git
cd limrun-v1-typescript
./scripts/bootstrap
./scripts/build
```
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npx -y limrun-v1-mcp`
...
```json
{
  "mcpServers": {
    "limrun_v1_api": {
      "command": "node",
      "args": ["/path/to/local/limrun-v1-typescript/packages/mcp-server", "--client=claude", "--tools=all"],
      "env": {
...
import { server, endpoints, init } from "limrun-v1-mcp/server";

// import a specific tool
import createAndroidInstances from "limrun-v1-mcp/tools/android-instances/create-android-instances";
...
- `retrieve_android_instances` (`read`): Get Android instance with given ID
...
- `retrieve_assets` (`read`): Get the asset with given ID.
